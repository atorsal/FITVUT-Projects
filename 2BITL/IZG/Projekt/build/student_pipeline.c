...
void gpu_runVertexPuller(
        ...
        GPUVertexPullerHead const *head = puller->heads;
        VertexIndex gl_VertexID = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);

        output->attributes[0] = head->enabled ? gpu_computeVertexAttributeDataPointer(head, gl_VertexID) : NULL;
        output->attributes[1] = (++head)->enabled ? gpu_computeVertexAttributeDataPointer(head, gl_VertexID) : NULL;
        output->attributes[2] = (++head)->enabled ? gpu_computeVertexAttributeDataPointer(head, gl_VertexID) : NULL;
        output->attributes[3] = (++head)->enabled ? gpu_computeVertexAttributeDataPointer(head, gl_VertexID) : NULL;
}
...
void gpu_runPrimitiveAssembly(
        ...
        GPUVertexShaderOutput *vertex = primitive->vertices;
        VertexIndex const* indices = puller->indices;

        for (VertexShaderInvocation offset = baseVertexShaderInvocation; offset < nofPrimitiveVertices + baseVertexShaderInvocation;  ++vertex, ++offset) {
                gpu_runVertexPuller(&out, puller, offset);
                in.attributes = (GPUVertexPullerOutput const*) out.attributes;
                in.gl_VertexID = gpu_computeGLVertexID(indices, offset);
                vertexShader(vertex, &in, gpu);
        }
