...
void gpu_runVertexPuller(
        ...
        void const *attr_data_pointer;
        VertexIndex gl_VertexID = gpu_computeGLVertexID(indices, vertexShaderInvocation);
        ...
        head = &puller->heads[0];
        if (head->enabled) {
                attr_data_pointer = gpu_computeVertexAttributeDataPointer(head, gl_VertexID);
                output->attributes[0] = attr_data_pointer;
        } else {
        ...
        if (head->enabled) {
                attr_data_pointer = gpu_computeVertexAttributeDataPointer(head, gl_VertexID);
                output->attributes[1] = attr_data_pointer;
        } else {
        ...
        if (head->enabled) {
                attr_data_pointer = gpu_computeVertexAttributeDataPointer(head, gl_VertexID);
                output->attributes[2] = attr_data_pointer;
        } else {
        ...
        if (head->enabled) {
                attr_data_pointer = gpu_computeVertexAttributeDataPointer(head, gl_VertexID);
                output->attributes[3] = attr_data_pointer;
        } else {
