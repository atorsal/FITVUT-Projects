...
/*                                                                                  */
/*  Reseni vytvoril(a) a odevzdava: (David Bolvansky, xbolva00)                                  */
/*                                                                                  */
/************************************************************************************/
...
void PORTB_IRQHandler(void) {
	...
	if (PORTB_ISFR & PB3_ISFR_MASK) { /* tlacitko NAHORU (Up) */
		/* zacatek odezvy */
		...
		PORTB->PCR[3] |= PORT_PCR_ISF(1);
	}
	else if (PORTB_ISFR & PB4_ISFR_MASK) { /* tlacitko PROSTREDNI (Central) */
		/* zacatek odezvy */
		...
		PORTB->PCR[4] |= PORT_PCR_ISF(1);
	}
	else if (PORTB_ISFR & PB5_ISFR_MASK) { /* tlacitko DOLEVA (Left) */
		/* zacatek odezvy */
		...
		PORTB->PCR[5] |= PORT_PCR_ISF(1);
	}
	else if (PORTB_ISFR & PB6_ISFR_MASK) { /* tlacitko DOLU (Down) */
		/* zacatek odezvy */
		...
		PORTB->PCR[6] |= PORT_PCR_ISF(1);
	}
	else if (PORTB_ISFR & PB7_ISFR_MASK) { /* tlacitko DOPRAVA (Right) */
		/* Zacatek odezvy */
...
void init_hardware(void) {
	...
	for (int i = 3; i <= 7; ++i) {
		PORTB->PCR[i] = PORT_PCR_ISF(1) | PORT_PCR_IRQC(10) | PORT_PCR_MUX(1) |
